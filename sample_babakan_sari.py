...
kelurahan_url = "https://sirekap-obj-data.kpu.go.id/wilayah/pemilu/ppwp/32/3273/327316.json"

response = requests.get(kelurahan_url)
data_kelurahan = response.json()

for item_kelurahan in data_kelurahan:    
     # Construct the URL with the correct paramCode
    tps_url = f"https://sirekap-obj-data.kpu.go.id/wilayah/pemilu/ppwp/32/3273/327316/{item_kelurahan['kode']}.json"
    ...
    for item_tps in data_tps:
        #print("ID:", item_kelurahan["id"])
        #print("Kode:", item_tps["kode"])
        # Construct the URL for detailed TPS data with the correct paramCode
        administration_result_url = f"https://sirekap-obj-data.kpu.go.id/pemilu/hhcw/ppwp/32/3273/327316/3273161005/{item_tps['kode']}.json"
            ...
            if(administration_result_data is not None and administration_result_data != '') :
                ...
                if(administrasi is not None and chart is not None): 
                    ...
                    if(suara_total < total):
                        print("--------")
                        print("Kecamatan : "+"Kiara Condong"+" Kelurahan :"+ item_kelurahan["nama"] +" " +item_tps["nama"]);
